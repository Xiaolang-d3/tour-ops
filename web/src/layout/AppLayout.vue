      ...
      <div class="logo" @click="router.push('/dashboard')">
        <div class="logo-icon">
          <svg viewBox="0 0 38 38" xmlns="http://www.w3.org/2000/svg" width="38" height="38">
            <defs><linearGradient id="pGrad" x1="0" y1="0" x2="38" y2="38"><stop stop-color="#7c3aed"/><stop offset="1" stop-color="#4f46e5"/></linearGradient></defs>
            <rect width="38" height="38" rx="10" fill="url(#pGrad)"/>
            <path d="M9 10.5 L30 19 L9 27.5 L13 19 Z" fill="#fff" opacity="0.95"/>
            <path d="M13 19 L30 19 L18 25 Z" fill="#e0e7ff" opacity="0.6"/>
          </svg>
        </div>
        <transition name="fade">
          <span v-show="!collapsed" class="logo-text">TourOps</span>
        </transition>
      </div>

      <div class="nav-section">
        <transition name="fade">
          <span v-show="!collapsed" class="nav-label">导航</span>
        </transition>
      </div>
          ...
          <el-dropdown @command="handleCommand" trigger="click">
            <div class="user-avatar-wrap">
              <el-avatar :size="34" class="user-avatar" :style="getAvatarStyle(userStore.user?.avatar)">
                ...
                <span v-else>{{ (userStore.user?.name || '用').charAt(0) }}</span>
              </el-avatar>
              <div class="user-info">
                <span class="user-name">{{ userStore.user?.name || '未设置昵称' }}</span>
                <span class="user-role">旅行规划师</span>
              </div>
...
const pageTitle = computed(() => {
  const map = { '/dashboard': '控制台', '/trips': '行程管理', '/templates': '模板库', '/ai': 'AI 助手' }
  if (route.path.startsWith('/trips/')) return '行程详情'
...
.collapse-btn:hover { color: #7c3aed; background: #f5f3ff; }
...
.mobile-menu-btn:hover { background: #f5f3ff; color: #7c3aed; }
